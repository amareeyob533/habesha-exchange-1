...
export async function GET(req: NextRequest) {
  ...
  if (!token) return renderPage('error', 'Missing approval token.')
  ...
  if (deposit.status === 'approved') {
    return renderPage('already', `Deposit of ${deposit.amount} ${deposit.token} (User ${deposit.user.uid}) was already approved.`, deposit)
  }
...
function renderPage(kind: 'success' | 'error' | 'already' | 'info', message: string, deposit?: DepositWithUser): NextResponse {
  const palette = {
    ...
    info: { color: '#F0B90B', title: 'Deposit', emoji: '●' },
  }[kind]
