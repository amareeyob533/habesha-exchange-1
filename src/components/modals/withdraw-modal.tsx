...
import { ArrowUpFromLine, Users, Globe, Loader2, ShieldAlert, Check, Info } from 'lucide-react'
      ...
      <DialogContent className="max-w-[460px] border-border bg-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <DialogDescription className="text-xs">Move funds internally or to an external wallet</DialogDescription>
          </div>
        </div>
...
function WithdrawForm({ initialSymbol, onClose }: { initialSymbol: string; onClose: () => void }) {
  ...
  const [done, setDone] = useState<null | { mode: 'internal' | 'external'; amount: number; to: string }>(null)
  ...
  // Force internal mode for Habesha (derived, no effect).
  const effectiveMode: 'internal' | 'external' = token?.internalOnly ? 'internal' : mode
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      await fetchMe()
      setDone({ mode: effectiveMode, amount: amt, to: effectiveMode === 'internal' ? targetUid.trim() : address.trim() })
    } catch (err: any) {
    ...
    <AnimatePresence mode="wait">
      {done ? (
        <motion.div key="done" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center py-6 text-center">
          ...
          <h3 className="mt-4 text-lg font-bold">{done.mode === 'internal' ? 'Transfer Complete' : 'Withdrawal Submitted'}</h3>
        ...
        <motion.form key="form" onSubmit={submit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <ModeButton active={effectiveMode === 'internal'} onClick={() => setMode('internal')} icon={Users} label="Internal Transfer" sub="By UID" />
            <ModeButton active={effectiveMode === 'external'} onClick={() => { if (!token?.internalOnly) setMode('external') }} icon={Globe} label="External Wallet" sub="Other exchange" disabled={!!token?.internalOnly} />
          </div>
          ...
          {token?.internalOnly && (
            <div className="flex items-start gap-2 rounded-lg border border-gold/30 bg-gold/5 p-2.5 text-[11px]">
              ...
              <span className="text-muted-foreground"><b className="text-gold">Habesha Token</b> can only be transferred internally between Habesha Exchange users. External withdrawals are not available.</span>
            </div>
          )}
          ...
          <div className="space-y-1.5">
            ...
            <Input type="number" min="0" step="any" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" className="bg-secondary/40" />
            <div className="flex items-center justify-between text-[11px]">
              <span className="text-muted-foreground">{amount && token ? `≈ ${formatUsd(Number(amount) * token.price)}` : ''}</span>
          ...
          <Button type="submit" className="bg-gold-gradient h-11 w-full font-semibold text-primary-foreground" disabled={loading}>
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : effectiveMode === 'internal' ? 'Send Instantly' : 'Submit Withdrawal'}
          </Button>
        </motion.form>
      )}
    </AnimatePresence>
