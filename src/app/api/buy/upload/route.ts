...
export async function POST(req: NextRequest) {
  try {
    ...
    let proof = null
    try { proof = await db.paymentProof.create({ data: { userId: user.id, mimeType, fileName: file.name || 'screenshot', data: dataUrl, size: file.size } }) }
    catch { await new Promise(r => setTimeout(r, 500)); proof = await db.paymentProof.create({ data: { userId: user.id, mimeType, fileName: file.name || 'screenshot', data: dataUrl, size: file.size } }) }
