...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!draftApp) {
      try { draftApp = await db.kycApplication.create({ data: { userId: user.id, fullName: '(draft)', city: '(draft)', idType: 'national_id', status: 'draft' } }) }
      catch { await new Promise((r) => setTimeout(r, 500)); draftApp = await db.kycApplication.create({ data: { userId: user.id, fullName: '(draft)', city: '(draft)', idType: 'national_id', status: 'draft' } }) }
    }
